...
class Translator:
    ...
    def detect_language(self, text):
        ...
        try:
            ...
            lang = detect(text)
            # 将越南语(vi)改为印尼语(id)
            return {'zh': 'chinese', 'id': 'indonesian'}.get(lang.split('-')[0], lang)
    ...
    def _get_translation_prompt(self, text, source_lang, target_lang):
        """获取翻译提示"""
        if source_lang == 'chinese' and target_lang == 'indonesian':
            return f"请将以下中文文本准确翻译成印尼语，保持口语化表达:\n\n{text}"
        elif source_lang == 'indonesian' and target_lang == 'chinese':
            return f"请将以下印尼语文本准确翻译成中文，保持口语化表达:\n\n{text}"
        else:
